...
> **Chatwoot v4.13.0** · Chart v3.3.48 · 严苛 CI/CD · 零停机升级 · 模块化安全治理
...
- **三级健康检测**：`startupProbe`（5.5 分钟启动窗口）→ `livenessProbe`（/health）→ `readinessProbe`（默认 /health，可显式切换 /api 深度依赖检查）
...
集成 `Lint → Unit Test (58 cases) → Release` 流水线。任何测试失败都将中止发布。
