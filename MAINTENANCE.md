...
当前测试基线：`12` 个 test suites，`64` 个用例。新增模板能力必须补充 helm-unittest，尤其是 selector、Service、Secret、PVC、probe、hook、镜像仓库覆盖、外部 Secret 和脱敏后的 Rancher 生产 Values。
...
5. 从 `3.3.38` 升级到 `3.3.56` 的预期变更仅限镜像版本、Web readiness 默认改回轻量 `/health`、探针参数渲染顺序、Web/Worker/迁移 Job 镜像缓存调度偏好、迁移 Job 的 PgBouncer/CNPG 与 Redis/Valkey 等待逻辑、CI 维护、镜像仓库说明，以及兼容性配置能力增强；Deployment selector、Service、Secret、PVC 名称必须保持不变
...
Chatwoot 官方镜像使用 `chatwoot/chatwoot:v4.13.0`。不要把 `ghcr.io/chatwoot/chatwoot` 当成官方镜像源；如果生产环境使用私有仓库、缓存仓库或 registry mirror，必须在 Rancher Values 中显式保留对应镜像仓库配置，避免升级时回落到 Chart 默认镜像引用。

```yaml
image:
  repository: chatwoot/chatwoot # 官方默认镜像；私有仓库可改为 registry.example.com/chatwoot
  tag: v4.13.0
...
Web、Worker 与迁移 Job 默认启用 `scheduling.preferExistingPods: true`，会软亲和到已有同类 Pod 所在节点，优先复用节点上的 `chatwoot/chatwoot:v4.13.0` 缓存镜像。这个设置不改变 `worker` + `longhorn` 节点硬约束，也不会强制绑定节点；如果生产环境已经通过自定义 affinity 完全接管调度，可显式关闭：
...
迁移 Job 仍保留更细粒度的 `hooks.migrate.preferExistingPods`，只在确实需要单独关闭迁移 Hook 的镜像缓存偏好时使用。历史 `storage.type: s3_compatible` 继续按旧兼容策略渲染为直接环境变量 `ACTIVE_STORAGE_SERVICE=local`，避免升级时改变线上存储行为；如需真正启用 S3 兼容存储，应先在测试环境验证 Chatwoot 启动和对象存储连通性，再单独调整。
...
- `revision 16` 首次升级失败原因：`wk-th-colo-01` 拉取当时渲染出的官方镜像引用 `chatwoot/chatwoot:v4.13.0` 超时/拒绝连接，应归类为该节点到官方仓库或内部 registry mirror 通道异常，不是迁移 Hook 或模板渲染错误
...
> 兼容性结论：Rancher 线上环境如果历史上手动缩放到 1 副本，后续升级必须在 Values 中显式保留 `web.replicaCount: 1`、`worker.replicaCount: 1`；如果使用私有镜像仓库，也必须保留对应 `image.repository`，避免 Chart 默认值触发额外 Pod 或错误镜像仓库拉取。
...
| 版本 | 关键变更 |
|------|----------|
| **v3.3.56** | Rancher 修复：Web、Worker 与迁移 Hook 默认软亲和到已有同类 Pod 所在节点，复用已缓存 `chatwoot/chatwoot:v4.13.0` 镜像，降低 Docker Hub 抖动导致的 `ImagePullBackOff`；恢复旧 `s3_compatible` 存储兼容行为，不在升级中改变线上 `ACTIVE_STORAGE_SERVICE`；测试扩展到 64 个用例 |
| **v3.3.55** | 已发布但不建议使用：仅覆盖迁移 Hook 镜像缓存复用，未覆盖 Web/Worker 滚动升级调度；同时尝试保留旧 `ACTIVE_STORAGE_SERVICE=s3_compatible`，线上验证发现会改变既有启动行为，已在 v3.3.56 回退 |
